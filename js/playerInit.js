...
function initializePlayer() {
  ...
  // 0️⃣ Apply any per-screen Pusher credentials saved at pairing time BEFORE the
  //    channels below are subscribed, so the first connection uses them.
  if (typeof configurePusher === "function") {
    configurePusher(pairedDevice.pusherKey, pairedDevice.pusherCluster);
  }
...
function fetchPlayerData(trackNew) {
  ...
  getPlayer(pairedDevice.screenId)
    .then(function (data) {
      ...
      if (data && data.pusher && typeof configurePusher === "function") {
        if (configurePusher(data.pusher.key, data.pusher.cluster)) {
          try {
            var pd = getPairedDevice();
            if (pd) {
              pd.pusherKey     = data.pusher.key     || pd.pusherKey     || null;
              pd.pusherCluster = data.pusher.cluster || pd.pusherCluster || null;
              savePairedDevice(pd);
            }
          } catch (e) { /* non-fatal */ }
        }
